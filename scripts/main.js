...
function validationURL() {
    var text = wibsite.value;
    
    var regex = /^(https?:\/\/)?([a-zA-Z0-9_-]+\.){1,}[a-zA-Z]{2,}([a-zA-Z0-9_-]+\/?)?$/

    if (regex.test(text)) {
        wibsite.classList.add("is-valid");
        wibsite.classList.remove("is-invalid");
        return true;
    } else {
        wibsite.classList.add("is-invalid");
        wibsite.classList.remove("is-valid");
        close();
        return false;
    }
}
...
function close ()
{
    lightBoxCountainer.classList.replace("d-flex" , "d-none")
}